...
export default defineConfig<PluginOptions>({
  testDir: './tests',
  /* Full e2e journeys (datasource + dashboard + query) need more than the 30s default
   * under a parallel run — 90s leaves headroom for firefox on a loaded machine */
  timeout: 90000,
  ...
  use: {
    ...
    /* Keep a trace for every failed test (locally there are no retries, so
     * 'on-first-retry' never produced one). See https://playwright.dev/docs/trace-viewer */
